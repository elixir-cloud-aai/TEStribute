...
class Request:
    ...
    def __init__(
        ...
        :param resource_requirements: Mapping of resources required for the
                ...
                Note that the `preemptible` and `zones` properties are
                currently not used.
        :param tes_uris: List of root URIs to known TES instances.
        :param object_ids: List of DRS identifiers of input files required for
                the task. Can be derived from `inputs` property of the
                tesTask` model of the GA4GH Task Execution Service schema
                described here:
        ...
        :param mode: Either a `mode.Mode` enumeration object, one of its
                members 'cost', 'time' or 'random', or one of its values 0, 1,
                -1, respetively. Depending on the mode, services are
                rank-ordered by increasing cost or time, or are randomized for
                testing/control purposes; it is also possible to pass a float
                between 0 and 1; in that case, the value determines the weight
                between cost (0) and time (1) optimization.

        :raises: TEStribute.errors.ValidationError
